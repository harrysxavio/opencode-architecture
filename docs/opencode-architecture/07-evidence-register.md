...
| E079 | D-T5 dry-run confirma Manager → gentle-orchestrator | ✅ VALIDADO RUNTIME | D4 post-change tests | task result | Manager invocó `gentle-orchestrator` como subagent para dry-run SDD; gentle devolvió envelope compacto; sin loop ni escritura funcional. | Fase D resuelve bloqueo T5 de Fase C. | Mantener prueba de regresión para SDD real. |
| E080 | D-T3 confirma rol objetivo de gentle desde Markdown | ✅ VALIDADO | `10-target-architecture.md`, `ADR-003`, `17-manager-gentle-transition-plan.md` | docs read | Documentación confirma gentle como SDD Pipeline invocable, no primary. | Markdown docs siguen como fuente de verdad. | — |
| E081 | Store real Engram identificado | ✅ VALIDADO | `C:\Users\harry\.engram\engram.db` | sqlite read-only | DB contiene observations=292, user_prompts=302, sessions=68, memory_relations=176. | Corrige diagnóstico previo basado en `.codex\memories_1.sqlite`. | Actualizar docs centrales. |
| E082 | Probe ficticio Engram persiste | ✅ VALIDADO | E1 TEST-E-MEMORY-GOVERNANCE | mem_save/mem_search/CLI/DB | id=395 guardado, recuperado, luego upserted a deprecated con revision_count=2. | mem_save funciona; metadata avanzada no es nativa. | Implementar gobernanza metadata si se aprueba. |
| E083 | mem_session_summary persiste como observation | ✅ VALIDADO | E1 TEST-E-MEMORY-GOVERNANCE | mem_session_summary + DB | id=396 creado como `type=session_summary`; `sessions.summary` queda vacío. | Los tests previos miraban el lugar incorrecto. | Ajustar validación. |
| E084 | Engram binarios/procesos duplicados | ✅ VALIDADO | runtime processes + where.exe | procesos/binaries | 3 procesos activos; PATH contiene v1.15.13 y v1.16.1 en otra ruta. | Riesgo de drift/config ambigua. | E4 proponer unificación. |
