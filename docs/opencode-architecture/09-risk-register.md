...
| R05 | **Guardado de ruido en memoria**: Prompt capture guarda prompts completos sin filtro | 🟢 **BAJO** | Baja (25%) | Memoria contaminada con datos transitorios, difícil retrieval semántico | E6B-D6: Noise Gate `classified` validado con smoke positivo/negativo | Ejecutar E6B-T1..T7 formal y ajustar heurísticas si aparecen falsos positivos/negativos | P2 |
...
| R20 | **Drift de project en sesiones Engram legacy**: sesiones existentes en `arquitectura opencode` chocan con project canónico `opencode-architecture` | 🟡 **MEDIO** | Alta (80%) | `/prompts` rechaza captura con `session_project_mismatch` en sesiones legacy | E6B-D6: primer intento post-restart siguió legacy y falló; sesión nueva canonical pasó smoke | Continuar en sesión nueva/canonical; no migrar automáticamente; si se decide consolidar, hacerlo en fase separada con backup y aprobación explícita | P2 |
...
| R19 | Plugins OpenCode incompatibles con runtime Node | Patch Node-compatible / aislar plugins con `Bun.*` | E6B-D6 | 🔄 Mitigado para `engram.ts`; pendiente `background-agents.ts` separado |
| R20 | Drift de project en sesiones Engram legacy | Sesión limpia + skip seguro; migración solo separada/aprobada | E6B-D6 | 🔄 Mitigado parcialmente; pendiente política de consolidación |
