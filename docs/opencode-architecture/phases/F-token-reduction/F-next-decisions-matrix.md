...
| **F4A — Skills Selective Loading** | ⏸️ Esperar aprobación | ~400–1,184 tokens | Medio — puede causar falsos negativos en skill matching | Sí — restaurar descripciones originales | ✅ Sí — requiere tocar config de skills | ❌ No | Revisar `F4A-skills-selective-loading-decision.md` y aprobar/rechazar |
...
1. **Lo más valioso ahora**: esperar compactación natural (F4B) — no requiere cambios, solo observación.
2. **Lo más seguro ya activo**: F4C guidance — runtime-validado y funcionando.
3. **Próximo candidato con mejor ROI**: F4A Skills (~400-1,184 tokens, riesgo medio). Requiere aprobación.
